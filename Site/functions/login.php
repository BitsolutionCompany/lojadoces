...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    try{
        // Preparar a consulta
        $stmt = $conn->prepare("SELECT * FROM user WHERE LOWER(email) = :username AND deletado = false");
        ...
        if($stmt->rowCount() > 0){
            ...
            if(password_verify($password, $results['password'])){
                $_SESSION['user'] = ["name" => $results['name'], "email" => $results['email'], "code" => $results['codeuser'], "status" => $results['status']]; // ou outro identificador
